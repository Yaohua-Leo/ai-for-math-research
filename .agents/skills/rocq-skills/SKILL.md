...
Before running Rocq or Coq commands in this workspace, read `tools/registry.yaml`
and use `scripts/run_formal_tool.py` or an experiment-local reproducible
command. Write proof sources, build logs, hashes, and command metadata under
`experiments/` before using any result as claim evidence. A Rocq build can be
`formal-check` evidence only for the checked proof files; this skill alone must
not promote computational or formal output to theorem status.
...
MCP + subagents + commands. Full workflow with interactive proof sessions, parallel tactic testing, and subagent dispatch. Subagents get pre-collected MCP context per [cycle-engine.md Section Pre-flight Context](references/cycle-engine.md#pre-flight-context-for-subagent-dispatch).
