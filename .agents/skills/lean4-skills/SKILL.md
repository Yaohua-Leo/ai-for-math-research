...
Before running Lean or Lake in this workspace, read `tools/registry.yaml` and
use `scripts/run_formal_tool.py` or an experiment-local reproducible command.
Write proof sources, build logs, hashes, and command metadata under
`experiments/` before using any result as claim evidence. A Lean build can be
...
haveI : MeasurableSpace Omega := inferInstance
letI : Fintype alpha := <...>
